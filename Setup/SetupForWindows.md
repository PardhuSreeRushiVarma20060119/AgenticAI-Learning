...
This creates a new directory `action` within your Projects folder and downloads the code for the class. Do `cd action` to go into it. This `action` directory is known as the "project root directory".
...
Type `pwd` to see the current directory, and check you are in the 'action' directory - like `C:\Users\YourUsername\Documents\Projects\action` or similar
...
Here's the thing: it **needs** to go in the directory named `action` and it **needs** to be named precisely `.env` -- not "env" and not "env.txt" or ".env.txt" but exactly the 4 characters `.env` otherwise it won't work!! 
...
3. In the field "Path to folder with a list of Virtual Environments" put the path to the project root, like C:\Users\username\projects\action
And then try again.

If you have any problems, I've included a Guide called [troubleshooting.ipynb](troubleshooting.ipynb) to figure it out.

Please do message me or email at ed@edwarddonner.com if this doesn't work or if I can help with anything. I can't wait to hear how you get on.
